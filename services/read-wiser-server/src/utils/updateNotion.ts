...
export const updateNotion = async (
  newClippings: {
    newClipping: boolean;
    content: string;
    date: string;
  ...
  const { id: booksDatabaseId } = results.filter(
    (result) => result.object === "database"
  )[0];

  const normalizedClippings = newClippings.reduce(
    (acc, val) => {
  ...
  const notionPagesToAdd = Object.keys(normalizedClippings).filter(
    (key) => !existingSourceList.includes(key)
  );

  await Promise.all(
    notionPagesToAdd.map(async (page) => {
  ...
  const fullyMapped = await Object.keys(normalizedClippings).reduce(
    async (acc, val) => {
