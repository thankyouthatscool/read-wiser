...
export const updateDatabase = async (clippings) => {
    const books = clippings.map((clipping) => {
        const { author, title } = clipping.source.match(/(?<title>.+)\((?<author>.+)\)$/)?.groups;
        return { author, title };
    });
    const sourceSet = books.reduce((acc, val) => {
        const targetBook = acc.find((book) => book.author === val.author || book.title === val.title);
        if (!targetBook) {
            return [...acc, { author: val.author, title: val.title.trim() }];
        }
        return acc;
    }, []);
    const fetchedClippings = await Promise.all(clippings.map(async (clipping) => {
    ...
    await Promise.all(betterFormattedClippings.map(async (clipping) => {
        try {
